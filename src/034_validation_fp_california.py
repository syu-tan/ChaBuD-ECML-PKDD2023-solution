...
Usage:
    $ python 034_validation_fp_california.py > fp_validation_california.log 2>  fp_validation_california.log
...
class CFG(object):
    ...
    outdir:str = f'output/val_fp_cal/034/v19-v15_th{TH_FIRE}_cloud-post{CLOUD_POST_ONLY}_no-cloud{NO_CLOUD_MODEL}_no-post{NO_POST_MODEL}/'
...
# inference
for cal_idx, (phase, PATH_DATA) in enumerate(zip(['public']*len(sub_cfg.PATHS_CV_DATA), sub_cfg.PATHS_CV_DATA)):
    with h5py.File(PATH_DATA, 'r') as fp:
        for num_idx, (uuid, values) in enumerate(fp.items()):
            ...
            with torch.no_grad():
                ...
                # post
                if not sub_cfg.NO_POST_MODEL or IS_DEFECT:
    ...
    if phase == 'public':
        ...
        NAME_CSV = f'034_{dt_now}_Cal{cal_idx}_f-th{sub_cfg.TH_FIRE:.2f}_c-th{sub_cfg.TH_CLOUD:.2f}_fp{fp_mean:.4f}_skip{sub_cfg.SKIP_THIS_FOLD}.csv'
