...
class CFG(object):
    ...
    # train info
    info += f'_fl-tr_WD{optimizer["params"]["weight_decay"]}E{epoch}GA{trainer["accumulate_grad_batches"]}'
    ...
    notebook: str = '025_chnage_model.py'
...
for it, img in enumerate(imgs):
    ...
    plt.axis('off')
plt.savefig(os.path.join(cfg.outdir, 'inference_analysis', f'025_post_image_visualize_tiles_error.png'))
plt.clf()
plt.close()
...
for it, img in enumerate(imgs):
    ...
    plt.axis('off')
plt.savefig(os.path.join(cfg.outdir, 'inference_analysis', f'025_pre_image_visualize_tiles_error.png'))
plt.clf()
plt.close()
...
plt.savefig(os.path.join(cfg.outdir, 'inference_analysis', f'025_mask_visualize_tiles_error.png'))
plt.clf()
plt.close()
...
plt.savefig(os.path.join(cfg.outdir, 'inference_analysis', f'025_mask-predict_visualize_tiles_error.png'))
plt.clf()
plt.close()
...
for idx_band in range(12):
    ...
    plt.savefig(os.path.join(cfg.outdir, 'inference_analysis', f'025_diff_mask_band{idx_band+1}_visualize_tiles_error.png'))
    plt.clf()
    plt.close()
    ...
    for it, (img, mask) in enumerate(zip(imgs, masks)):
        ...
        plt.imshow(pred, alpha=0.4, cmap='jet', vmax=0.8)
        plt.imshow(mask.numpy(), alpha=0.4, cmap='winter')
    plt.savefig(os.path.join(cfg.outdir, 'inference_analysis', f'025_overlay_band{idx_band+1}_visualize_tiles_error.png'))
    plt.clf()
    plt.close()