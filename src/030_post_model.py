...
class CFG(object):
    ...
    # SMOOTHING_UNDER_RANGE = (0.01, 0.1)
    BCE_POS_WEIGHT = 1
    NUM_UP_SAMPLING = 4
    ...
    # train info
    info += f'_fl-tr_{optimizer["params"]["lr"]}_E{epoch}'
    # info += 'fp16'
    info += f'CE-W{BCE_POS_WEIGHT}UP{NUM_UP_SAMPLING}'
    ...
    notebook: str = '030_post_model.py'
...
for state in ['post', 'pre']:
    ...
    if state == 'post':
        for _ in range(cfg.NUM_UP_SAMPLING):
            dfs.append(df)
    else: 
        dfs.append(df)
...
# Idaho Data
df = pd.read_csv(cfg.PATH_CSV_IDAHO)
df['h_idx'] = df['name'].apply(lambda x: int(x.split('_')[1][1:]))
df['w_idx'] = df['name'].apply(lambda x: int(x.split('_')[2][1:]))
df['post_missing'] = False
df['pre_missing'] = False
...
class FireDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if row.state == 'pre':
            img = np.load(PATH_FEAT).astype(np.float32)[:, :, 12:] # pre feature
            mask = np.zeros((img.shape[:2]), dtype=np.float32)
            
        elif row.area in ['tokyo', 'nagoya', 'Arizona', 'Nevata', 'Oregon', 'Idaho']:
            ...
            img = np.load(PATH_FEAT).astype(np.float32)[:, :, :12] # post feature
...
#     # plt.axis('off')
# plt.savefig(os.path.join(cfg.outdir, 'artifact', f'030_image_visualize_tiles_val.png'))
# plt.show();
...
# plt.savefig(os.path.join(cfg.outdir, 'artifact', f'030_mask_visualize_tiles_val.png'))
# plt.show();
...
#     # plt.axis('off')
# plt.savefig(os.path.join(cfg.outdir, 'artifact', f'030_image_visualize_tiles_train.png'))
# plt.show();
...
#     plt.subplot(4, 4, it+1)
#     plt.imshow(mask.numpy())
# plt.savefig(os.path.join(cfg.outdir, 'artifact', f'030_mask_visualize_tiles_train.png'))
# plt.show();
...
for fold in range(cfg.folds):
    ...
    # train val split 
    train_df = df[df['cv'] != fold].reset_index(drop=True)
    ...
    val_df = val_df[(val_df['defects'] == False) & (val_df['fold'] != -1) & (val_df['state'] == 'post')].reset_index(drop=True) # always save validation score tracing
...
for fold in range(cfg.folds):
    ...
    val_df = df[(df['cv'] == fold) & (df['state'] == 'post')].reset_index(drop=True)
    val_df = val_df.drop_duplicates(subset='name')
    ...
    for i, (images, labels) in enumerate(dl_val):
        ...
        postimg = images[0, 3:6, :, :] # B, C:12, H, W
...
for it, img in enumerate(imgs):
    ...
    plt.axis('off')
plt.savefig(os.path.join(cfg.outdir, 'inference_analysis', f'030_post_image_visualize_tiles_error.png'))
# plt.show();
plt.clf()
plt.close()
...
for it, mask in enumerate(masks):
    ...
    plt.title(uuid)
    plt.imshow(mask.numpy())
plt.savefig(os.path.join(cfg.outdir, 'inference_analysis', f'030_mask_visualize_tiles_error.png'))
# plt.show();
plt.clf()
plt.close()
...
plt.savefig(os.path.join(cfg.outdir, 'inference_analysis', f'030_mask-predict_visualize_tiles_error.png'))
# plt.show();
plt.clf()
plt.close()
...
for idx_band in range(12):    
    ...
    plt.savefig(os.path.join(cfg.outdir, 'inference_analysis', f'030_overlay_band{idx_band+1}_visualize_tiles_error.png'))
    plt.clf()
    plt.close()
