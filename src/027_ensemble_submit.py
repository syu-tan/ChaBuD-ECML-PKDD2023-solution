...
class CFG(object):
    ...
    # data
    # PATH_SUBMIT_DATA:str = f'../data/train_eval.hdf5'
    PATH_SUBMIT_DATA:str = f'../data/test.h5'
    
    # models
    PATH_MODEL_ROOT_CHANGE:str = f'output/V14/A100_V14_CALFalseDEFFalseNONTrue_hrnet_w30BC16TH0.3_R-RPM_R-RPM_fl-tr_WD0.001E15GA1CE-W1UP4BETA0.25R0.5MIX0.5FREE12B32_Scr'
    PATH_MODEL_ROOT_POST:str   = f'output/V15/A100_V15_CALFalseDEFFalseNONTrue_hrnet_w30BC16TH0.3_R-RPM_fl-tr_0.0001_E15CE-W1UP4BETA0.5R0.1MIX0.5FREE12B32_Scr'
    PATH_MODEL_ROOT_CLOUD:str  = f'output/cloud/V2/V100_V2_C_resnet34BC4TH0.4_LR0.01T0100MIX0.5B16_fl-tr'
    
    TH_FIRE:float  = 0.3
    TH_CLOUD:float = 0.75
    
    outdir:str = f'output/inference/027/'
...
with h5py.File(sub_cfg.PATH_SUBMIT_DATA, 'r') as fp:
    for num_idx, (uuid, values) in enumerate(fp.items()):
        if (values.attrs['fold'] != sub_cfg.fold) and not sub_cfg.IS_SUBMIT:
...
if not sub_cfg.IS_SUBMIT:
    ...
    NAME_CSV = f'027_train_val_f-th{sub_cfg.TH_FIRE:.2f}_c-th{sub_cfg.TH_CLOUD:.2f}_fold{sub_cfg.fold}_iou{iou_mean:.4f}.csv'
...
if sub_cfg.IS_SUBMIT:
    NAME_CSV = f'027_test_f-th{sub_cfg.TH_FIRE:.2f}_c-th{sub_cfg.TH_CLOUD:.2f}.csv'
